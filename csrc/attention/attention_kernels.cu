...
template<
  ...
  int NUM_THREADS,
  int PARTITION_SIZE = 0> // Zero means no partitioning.
inline __device__ void custom_attention_kernel(
  float* __restrict__ exp_sums,           // [num_seqs, num_heads, max_num_partitions]
  float* __restrict__ max_logits,         // [num_seqs, num_heads, max_num_partitions]
  scalar_t* __restrict__ out,             // [num_seqs, num_heads, max_num_partitions, head_size]
  const scalar_t* __restrict__ q,         // [num_seqs, num_heads, head_size]
...
#pragma unroll
      ...
      // Compute dot product.
      // This includes a reduction across the threads in the same thread group.
      float qk = scale * Qk_hmma_dot<scalar_t, THREAD_GROUP_SIZE>::dot(q_vecs[thread_group_offset], k_vecs);
...
__global__ void paged_attention_v3_kernel(
  ...
  const int kv_block_stride,
  const int kv_head_stride) {
  custom_attention_kernel<scalar_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, PARTITION_SIZE>(
    exp_sums, max_logits, tmp_out, q, k_cache, v_cache, head_mapping, scale, block_tables,
    context_lens, max_num_blocks_per_seq, q_stride, kv_block_stride, kv_head_stride);
}
...
#define LAUNCH_PAGED_ATTENTION_V3(HEAD_SIZE)                                                  \
  vllm::paged_attention_v3_kernel<T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, PARTITION_SIZE>      \
  <<<grid, block, shared_mem_size, stream>>>(                                                 \
    exp_sums_ptr,                                                                             \
    max_logits_ptr,                                                                           \
    tmp_out_ptr,                                                                              \
...
void paged_attention_v3(
  ...
  int max_context_len) {
  if (query.dtype() == at::ScalarType::Float) {
    CALL_V3_LAUNCHER_BLOCK_SIZE(float);
  } else if (query.dtype() == at::ScalarType::Half) {
    CALL_V3_LAUNCHER_BLOCK_SIZE(uint16_t);
  } else if (query.dtype() == at::ScalarType::BFloat16) {
    CALL_V3_LAUNCHER_BLOCK_SIZE(__nv_bfloat16);
  } else {
    TORCH_CHECK(false, "Unsupported data type: ", query.dtype());
  }
}
